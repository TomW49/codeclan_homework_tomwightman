...
#Could give measurement units for run times, revenues, budget.
#Title columns do have differences, but need to chose exact title of film as to cut down on columns
#Language could be in full rather than abbreviated 
#Can use data to see best/worst profit, most popolar film of a time era etc
...
```{r}
movie_budgets_imputed <- movies_imputed %>% 
  mutate(budget = coalesce(budget,
                            median(runtime, na.rm = TRUE)))

movie_budgets_imputed %>%
  filter(is.na(budget))
  
#this was a copy and paste of other code, tweaked it but not enough 
```
...
Hint:
1e6 is a shorthand way of writing 1 million ()
...
```{r}
movies %>% 
  summarise(is.na(across(original_title:vote_count))
