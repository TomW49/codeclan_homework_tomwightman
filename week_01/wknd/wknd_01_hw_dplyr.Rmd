...
# have noticed that some books may have been removed from the dataset, as "rowid" and "bookID" do not match

books %>% 
  summarise(across(everything(), 
                   .fns = ~ sum(is.na(.x))))
# investigating the dataset, code 24-26 details if and NA within data - which there is not
```
...
books %>% 
  group_by(authors) %>% 
  summarise(average_rating >= 17942.85 )
  
# aiming to only compare authors of have an average rating count higher than the average rating count for all books  
```
