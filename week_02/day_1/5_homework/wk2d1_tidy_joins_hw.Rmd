...
inner_join(transaction_base, fraud_base, by = "transaction_id")
```
Row number tells me there are only 109 fraud cases linked to held transaction details
...
hat_obs%>% 
  unite(date, 
        ...
        sep = "/") %>% 
  group_by(date) %>% 
  summarise(hat_type = n()) %>% 
  slice_max(hat_type == "beret")
```
...
exam_scores %>%
    pivot_longer(-1, 
               ...
               values_to = "score") %>% 
  separate(exam_question, c("exam", "exam_question"), sep = "_" ) %>% 
