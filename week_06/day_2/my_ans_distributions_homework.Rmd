...
```{r}
data <- clean_names(read_csv("data/20190928-items.csv"))
...
## Question 2.
Find the brand with the highest numbers of phones in the data set.
```{r}
...
```{r}
mobile_data_1_sd <- data %>%
  filter(rating >= samsung_stats$mean - samsung_stats$sd) %>%
  filter(rating <= samsung_stats$mean + samsung_stats$sd) %>%
  summarise(prop_within_1sd = n() / nrow(data))
mobile_data_1_sd 
```
